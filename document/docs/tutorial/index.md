...
chrome.tabs.executeScript(tabId, {file: 'some-script.js'});` 复制运行
```
...
chrome.tabs.insertCSS(tabId, {file: 'some-style.css'});` 复制运行
```
...
}, {urls: ['<all_urls>']}, ['extraHeaders', 'responseHeaders']);` 复制运行
```
...
chrome.cookies.remove({url, name: cookieName}, details => {});` 复制运行
```
